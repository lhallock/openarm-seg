...
def train(sess,
    ...
    summarywriter = tf.summary.FileWriter("./logs/1/train/", sess.graph)
...
#             merge = tf.summary.merge_all()
            summary, loss, loss_summary = model.fit_batch(sess,x_train_temp, y_train_temp)
...
#             if summary_writer:
            summarywriter.add_summary(loss_summary, step)
            if len(losses) == 20:
                losses.popleft()
            losses.append(loss)
            stop = timeit.default_timer()
